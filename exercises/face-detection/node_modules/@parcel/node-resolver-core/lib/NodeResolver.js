    ...
    while (dir !== root) {
      ...
      try {
        ...
        if (stats.isDirectory()) {
          let dirContent = await this.fs.readdir(modulesDir); // Filter out the modules that interest us
          ...
          if (isOrganisationModule) {
            await Promise.all(modules.map(async item => {
              ...
              let orgDirContent = await this.fs.readdir(orgDirPath); // Add all org packages
